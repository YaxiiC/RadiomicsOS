# === Inception v3 3-Class Classifier: Patient-Level Split (Train: Case-3/P9/Case-48, Val: Case-48 slice, Test: Case-4) ===
...
VAL_FRACTION_FROM_VALPAT = 0.10                 # small validation slice fraction
TRAIN_PATIENTS = ["Case-3", "P9", "Case-48"]    # Train set patients
VAL_FROM_PATIENT = "Case-48"                    # Take val slice from Case-48
TEST_PATIENT = "Case-4"                         # Hold out all of Case-4 for test

# --- Regularization / Dropout config --------------------------
DROP_RATE = 0.30            # <<< standard dropout inside timm head
DROP_PATH_RATE = 0.10       # <<< stochastic depth
...
class CNNClassifier(nn.Module):
    def __init__(self, backbone: str, num_classes: int):
        ...
        # only add supported arguments
        if "inception" not in backbone.lower():
            kwargs.update(dict(drop_path_rate=DROP_PATH_RATE))
        kwargs.update(dict(drop_rate=DROP_RATE))  # works for inception_v3
...
def train_model_once(seed=42):
    ...
    # --- Fixed per-label weights: 3.0 for each class ---
    class_weights = torch.tensor([3.0] * num_classes, dtype=torch.float32).to(DEVICE)
...
def evaluate_model_5x(model, test_df, classes, seeds_eval=MC_DROPOUT_EVAL_SEEDS):
    ...
    # Fixed test loader (order doesn't matter since we compute metrics, but keep it stable)
    ...
    for s in seeds_eval:
        ...
        # Enable ONLY dropout layers to make a stochastic forward pass
    ...
    # After runs, restore normal eval mode
...
if __name__ == "__main__":
    model, df, classes, test_df = train_model_once(seed=42)  # train ONCE (with dropout & drop-path)
